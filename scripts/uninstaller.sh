...
main() {    
    ...
    CMD_PATH="/usr/local/bin/winamp"
    ENTRY_PATH="/home/$USER/.local/share/applications/winamp.desktop"
    
    notify-send "WinampLinux" "Winamp uninstaller started" -i "winamp"

    ask_question "Are you sure you want to UNINSTALL winamp?" "N"
    if [ $result == "no" ];then
        echo "Ok good Bye :)"
        exit 0
    fi
    ...
    if [ -d "$SCR_PATH" ];then
        ...
        echo "winamp directory Not Found!"
    fi
    
    
    #Unlink command 
    if [ -L "$CMD_PATH" ];then
        echo "remove winamp command..."
        sudo unlink "$CMD_PATH" || error2 "couldn't remove winamp command"
    else
    ...
    if [ -d "$CACHE_PATH" ];then
        echo "--------------------------------"
        echo "all downloaded components are in cache directory and you can use them for winamp installation next time without wasting internet traffic"
        ...
        if [ "$result" == "yes" ];then
            ...
            echo "nice, you can use downloaded data later for winamp installation"
        fi
    else
